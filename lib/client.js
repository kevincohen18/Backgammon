...
function Client(config) {
  ...
  this._openSocket = function () {
    ...
    if (!serverURL) {
      serverURL = window.location.origin || window.location.host;
    }
  ...
  /**
   * Handle reply - New bot match has been created
   * @param {Object} params - Message parameters
   */
  this.handlePlayBot = function (params) {
    if (!params.result) {
      return;
    }
    console.log('Created bot match with ID ' + params.match.id + ' and rule ' + params.ruleName);
  ...
  /**
   * Request playing a match against a bot.
   * @param {string} ruleName - Name of rule to use (eg. RuleBgCasual)
   * @param {messageCallback} [callback] - Callback function to be called when server sends a reply
   */
  this.reqPlayBot = function (ruleName, callback) {
    this.sendMessage(
      comm.Message.PLAY_BOT,
      {
        'ruleName': ruleName
      },
      callback
